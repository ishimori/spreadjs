...
- **grid `GridInstance.setRows`（DD-052-4・ReadyCrew RC4）**: 単独グリッドモード専用。`setData` と違い渡した行
  ...
  バイパスする。
- **grid `header-click` イベント（DD-052-5・ReadyCrew RC5）**: 列記号ヘッダーのクリック通知（両モード共通）。SDK は
  並べ替えを行わず `columnId` を通知するのみ。既存のセルクリック（選択・編集開始）の挙動は変えない。
- **grid `cell-hover` イベント（DD-052-5・ReadyCrew RC6）**: セルのホバー通知（両モード共通）。ポインタの位置する
  セルが変わったときだけ発火し（同一セル内の移動では再発火しない）、セルの viewport 矩形（`GridCellRect`）を運ぶ。
  SDK 自体はツールチップ等を描画しない（利用側が `rect` を使って自前で描く）。グリッド外・見出し領域・ドラッグ中は
  `rowId`/`columnId`/`rect` が `null` のイベントで通知する。
...
- **配布**: 10 package を `0.1.0-alpha.8` へ更新。API 版は既存 `0.1.0-experimental` を維持する。実装完了後に Codex
  CLI（xhigh・`doc/DD/DD-052/codex-review-result.md`）で差分全体をレビューし、指摘7件（P1×2・P2×5）を採用・修正済み。
...
- **配布**: 10 package を `0.1.0-alpha.6` へ更新。API 版は既存 `0.1.0-experimental` を維持する。
