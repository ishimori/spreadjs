...
async function measure(browser: Browser, rows: number, cols: number, borders: boolean) {
  const context = await browser.newContext({ viewport: { width: 1280, height: 800 }, deviceScaleFactor: 1 });
  const page = await context.newPage();
  try {
    const params = new URLSearchParams({ extracols: String(cols - 4), frozencols: '5', frozenrows: '1' });
    if (borders) {
      params.set('colborder', Array.from({ length: Math.floor((cols - 4) / 2) }, (_, i) => `col-x${i * 2 + 1}:right:2:64748b`).join(';'));
    ...
    const result = await page.evaluate(async ({ rows, cols }) => {
      ...
      return { rows, cols, frameCount: sorted.length, frameP95Ms: sorted[Math.ceil(sorted.length * 0.95) - 1] ?? 0, worstMs: sorted.at(-1) ?? 0 };
    }, { rows, cols });
