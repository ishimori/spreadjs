...
| DD | 件名 | ステータス | 理由 |
|----|------|-----------|------|
| DD-049-1 | 変更者の可視化 | 見送り | consumer（広島 DD-005）は H3 を「作らない・変更履歴で見せる」で確定し SDK 側の着手要望なし（ユーザー判断 2026-09-12）。再要望時は本DDの論点表を土台に再起票 |

## 完了済み

| DD | 件名 | 主な成果 |
|----|------|---------|
| DD-055 | 候補欄のはみ出しと長文編集欄の高さ | consumer 駆動（ReadyCrew DD-124）。要件正本は ready_crew_db `doc/DD/DD-124/sdk-requirements.md` RC16・RC17。候補欄（選択式・日付）を可視域に収め、高い行の編集欄がセル全体を覆うよう修正。Codex high 指摘なし。0.1.0-alpha.10で配布済み |
...
| DD-046 | セル移動時のスクロールバー遮蔽 | 可視域判定を修正。alpha.3を松下へ導入し根拠セル移動を実画面確認 |
