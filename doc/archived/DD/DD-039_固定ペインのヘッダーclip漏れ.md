...
| # | 項目 | 正味 | 結果 |
|---|------|------|------|
| M1 | 固定列 5・固定行 1 の実機での見え方（横／縦スクロール中にヘッダーが読めること・境界での文字切れが不自然でないこと） | 3 分 | ✅ **実施済み（2026-09-04）**。前半（ヘッダーが読める・固定帯が不変）は PASS。後半（文字切れ）は**不自然に見える実例を 1 件観測** → DD-042 へ起票。証跡は `DD-039/manual-gate-m1-clip-sweep.json`・`manual-gate-m1-boundary-readable.png`／`manual-gate-m1-boundary-misread.png` |

## 既知の未保証境界

- ~~**M1（実機での見え方）は未実施**~~ → **2026-09-04 に後追い実施**（証跡は `DD-039/manual-gate-m1-clip-sweep.json` ほか）。結果は次の 2 点に分かれた。
  - **固定側ヘッダー帯の不変は実機 DPR でも成立**（PASS）。DPR 2 で scrollLeft を 0 / 37 / 113 / 260 / 777 / 5000 / 20000 と変えても、固定側ヘッダー帯のスクリーンショットの SHA-256 は**7 通りすべて同一**。縦スクロール中もヘッダーは読める
  - **「境界で切れた漢字が別字に見える」は実在した**（本 DD の起票時に懸念として書いた通りの現象）。列幅 80px を 4px 刻みでスイープすると、`納入回数` の `納` が糸偏の位置で切れ「**内入回数**」と読める scrollLeft が存在する（DPR 3 で確認・`manual-gate-m1-boundary-misread.png`。4px ずらすと `勺入回数` と明らかな断片になり、8px 手前では `納入回数` と正しく読める＝`manual-gate-m1-boundary-readable.png`）。**列幅の 5〜10% 程度の狭いスクロール位置でのみ成立**する
  - この現象は**表示のみで、値・編集・計算・ヒットテストには影響しない**（本 DD の修正が直した「固定ペインへの重なり」とは別物で、重なりは解消済み）。対処（省略記号を出す／固定境界にかかる見出しを描かない等）は **DD-042 で扱う**
- **Presence の activeCell 枠・名前タグは本 DD の対象外**（`overlay-layer.ts` は無改変）。同じ「pane 境界を跨ぐ単一 clip」の欠陥が残っており、**DD-041** で扱う
...
- **Codex medium レビュー（ユーザー指示・`--commit dbd4947`）: findings 0**。「固定帯とスクロール帯の clip 分割は負幅・全列固定・固定帯が viewport を超える場合も安全に処理され、Canvas 状態管理も対称」＝**反映すべき指摘なし・見送り 0**（記録: `DD-039/codex-review-request.md` / `codex-review-result.md`）
- 知見昇格: `doc/engineering-patterns.md` #17（pane ごとの clip を描画の一部にだけ適用すると overscan と境界要素で必ず漏れる）・#18（後追いで書いた回帰テストは修正前コードのコピーに当てて red を確認する）
- **クローズ**（ユーザー指示: Codex レビュー完了をもって完了として可）。M1 未実施ぶんは「既知の未保証境界」へ移送済み
