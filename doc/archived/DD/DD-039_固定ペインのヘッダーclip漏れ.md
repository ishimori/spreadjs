...
| # | 項目 | 正味 | 結果 |
|---|------|------|------|
| M1 | 固定列 5・固定行 1 の実機での見え方（横／縦スクロール中にヘッダーが読めること・境界での文字切れが不自然でないこと） | 3 分 | 未実施（クローズ非ブロック＝下記へ移送） |

## 既知の未保証境界

- **M1（実機での見え方）は未実施**＝保証しているのは「固定側のヘッダー帯の実ピクセルがスクロールで変化しないこと」（Playwright・ヘッドレス Chromium）まで。**未確認なのは「固定境界をまたぐ見出しの文字切れが、実機のフォント・DPR で不自然に見えないか」**（例: 半端な位置で切れた漢字が別字に見える等）。問題が出ても表示のみで、値・編集・計算・ヒットテストには影響しない。実機で不自然だった場合は別 DD で扱う（省略記号を出す／固定境界にかかる見出しを描かない等の選択肢がある）
