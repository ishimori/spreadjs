...
- **元DD**: DD-002（5569375 で修正）→ DD-008 で再発（b2e8c69 で修正・本パターンに昇格）→ DD-052（6DD一括アーカイブ時に再発。`git diff --cached` で全ファイルの staged 内容を確認して発見・`git add` で是正）
...
## 22. 点線を既定の実線へ重ねても隙間はできない（背景を消さず格子線の段階で抑止する）

- **症状**: Canvasグリッドに点線を上描きしても隙間に既定格子線が残る。白塗りで消すと行・列・値ベース背景が欠ける。
