...
| ~~K4~~ | ~~実IME変換中に対象行が削除された場合の挙動~~ | roadmap §8 | **DD-021-2 で回収済（synthetic・2026-07-17）** | 編集継続・draft/composition 非破壊・commit 時退避＋公開 rejected（row-unavailable）通知。**実IME 確認（DD-021 Manual Gate M1）は未実施のまま 2026-09-03 にユーザー判断でクローズ**＝synthetic 三層までが証拠水準（DD-021 §既知の未保証境界） |
...
| K5 | client-sequence-violation 受信時の再送（`session.ts` handleRejected）が一部のフォールト系列で収束せず、再送→violation→再送のループが止まらないケースがある（`client-sequence 完全再整列は未実装`＝既存コメントの D27 deferred 境界）。DD-053 の presence 変更で偶然 RNG 系列がずれ、既存の固定 seed（1,234,567）がこの症状を引くようになって発見。同一 config で seed を 1〜40 に振ると **40件中10件（25%）が同じ message storm** で収束不能（`InProcessHub.deliverAll` の安全上限＝100,000 steps で強制停止） | DD-015（reconnect-fault 不変条件を追加した回） | **未回収（Stage 2/3 候補・要起票）** | `injectClientToServer:true`（client→server の submitOperation 欠落を注入する経路）でのみ確認。`rebaselinePendingSequence`（expected<firstUnacked のケース）は既に実装済みだが、それ以外の「通常の欠落＝serverがclientより進む」経路の再送が violation を再生産するケースを塞げていない可能性が高い。修正は Sequencer/ClientSession の protocol level 変更を伴いうる＝着手時に Risk Class A 相当で再判定。当面はテスト側で該当 seed を回避（`tests/invariants/collab/reconnect-fault.invariant.test.ts` のコメント参照） |
...
## 3.7 consumer 要望（広島空港 予算共同編集・2026-09-12・DD-049 で仕分け）

> **出典**: 広島空港モック（`C:/repo/hiroshima-airport` DD-005 共同編集の本物化）の SDK 要件メモ `doc/DD/DD-005/sdk-requirements.md`（H1〜H10）。
> DD-049 で H2（受理通知 `onAccepted`）・H4（`remote-change`）・H5（参加者一覧 `presence`/`presences()`）・H8（紹介サイト表記）を提供済み。
> H10（セルを選ばずに参加しただけの利用者が参加者一覧に載らない不整合。DD-005-3 で発見）は DD-053 で提供済み
> （「参加」＝join 完了時点として `presence`/`presences()` 契約 §4 を明確化＝Fixed。wire メッセージの種別・payload の形は変更なし）。
> 残りの回収先を以下に記録する（本表は出典記録。設計・起票は各回収先で行う）。

| 項目 | 内容 | consumer のつなぎ | 回収先 |
|---|---|---|---|
| H1 数式・集計 | 行 SUM・列 SUM・差分（行の増減で参照が壊れない） | サーバーが `onAccepted` で再計算し `ServerInstance.submit` で読み取り専用列・行へ書き戻す | DD-022（数式DD・§1） |
| H3 変更者の可視化 | セルを最後に変えた人を色・ツールチップで示す（自分／相手の区別） | `remote-change`（actorId・origin）から利用側で表示 | **見送り**（DD-049-1 を 2026-09-12 に見送りでアーカイブ。consumer は変更履歴〔`remote-change`〕で代替し、印の口の要望なし。再要望時は DD-049-1 の論点表〔view-local の印の口・書式／Presence 描画との重なり順〕を土台に再起票） |
| H6 セルコメント | セル右上のマーク＋ホバーで本文 | 作らない（「コメントは今後確認する範囲」と画面に明記） | セル単位の書式モデル（§3.5）と統合して設計 |
| H7 行の編集権限（サーバー側検査） | actor→編集可能行をサーバーで検査し、拒否は既存の reject 経路 | 全員が全行を編集できる状態で見せる（`readOnlyRows` は view-local で権限ではない） | 将来スコープ（松下 R4 の列権限と同じ扱い）。actor×行/列の認可モデル設計が要る＝Human Spec Gate 規模 |

## 3.8 ReadyCrew 商談進捗要望・範囲外（DD-052・2026-09-12）

> **出典**: ReadyCrew 案件DB 商談進捗シートの SDK 要件メモ（ready_crew_db `doc/DD/DD-124/sdk-requirements.md` RC1〜RC14）。
> DD-052 の Human Spec Gate 論点1で、高・中優先（RC1〜RC6・RC12〜RC14）は子DD化（DD-052-1〜5）、低優先（RC7〜RC11）は本節に記録し consumer のつなぎで運用する方針とした（推奨採用・2026-09-12）。

| 項目 | 内容 | consumer のつなぎ | 再検討条件 |
|---|---|---|---|
| RC7 select の value/label 分離 | `options: readonly string[]` は表示値と保存値が同一 | consumer 側で表示用マッピングテーブルを保持し、`setData` 前後で変換 | 複数実案件で要求されたら列タイプ体系（§3.6）と合わせて設計 |
| RC8 2段見出し | `columnCaptions` は1段のみ（DD-033 D4 で対象外判断済み） | consumer 側で列名にグルーピングを埋め込む（例:「商談/金額」） | DD-033 D4「複数実案件で要求されたら別DD」を維持。ReadyCrew 1件では覆さない |
| RC9 セルのクリック通知（link 以外） | `link-open` 以外のクリック通知イベントがない | 当面不要（RC5 の `header-click` で代替できる範囲のみ利用） | 具体的な用途が出たら RC5/RC6 と合わせて通知系イベントの追加として起票 |
| RC10 確定前の検証フック | `onCellCommit` は通知のみで確定を止められない | consumer 側は確定後に誤りをハイライト表示する等の事後対応 | DD-033 D4 と同種の「通知のみの契約」を覆す実例が複数出たら再検討 |
| RC11 Vite 8 での導入 | SDK は Vite 6・広島モックは Vite 7 で検証済み。ReadyCrew の環境は未確認 | consumer（DD-124 Phase 2）が実際に組み込んでビルド結果を確認し、問題があれば個別報告 | ReadyCrew の実導入時に Vite 8 起因の問題が出たら起票 |
