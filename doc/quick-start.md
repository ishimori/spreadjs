...
- **列見出しクリック・セルホバー（DD-052-5）**: `header-click{columnId}` は列記号ヘッダーのクリック通知（並べ替え自体は
  行わない・利用側が実装する）。`cell-hover{rowId,columnId,rect}` はポインタの位置するセルが変わるたびに届く
  （グリッド外・見出し領域・ドラッグ中は全て `null`）。`rect` はセルの viewport 矩形で、ツールチップ等は利用側が
  自前で描画する（SDK は描画しない）。両方とも両モード共通。
...
- 編集中の常駐 textarea が入力内容に合わせて下へ伸び、一定の高さ（8 行相当。行の高さがそれより高いセルはセルの高さ）を
  超えると内部スクロールに切り替わる。可視域の下端を越える場合は下端までに縮めて内部スクロールにする（DD-055）。
  列幅は変わらない。
